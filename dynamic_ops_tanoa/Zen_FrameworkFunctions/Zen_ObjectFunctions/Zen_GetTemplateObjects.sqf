...
_Zen_stack_Trace = ["Zen_GetTemplateObjects", _this] call Zen_StackAdd;
...
if (count _indexes < 1) exitWith {
    ZEN_FMW_Code_ErrorExitValue("Zen_RemoveTemplate", "Invalid template name", [])
};
...
if (count _indexes < 1) exitWith {
    ZEN_FMW_Code_ErrorExitValue("Zen_RemoveTemplate", "Invalid spawned template name", [])
};
