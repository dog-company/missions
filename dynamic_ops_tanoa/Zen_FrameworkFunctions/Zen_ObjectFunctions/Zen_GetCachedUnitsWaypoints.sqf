...
_Zen_stack_Trace = ["Zen_GetCachedUnitsWaypoints", _this] call Zen_StackAdd;
...
if (count _indexes == 0) exitWith {
    ZEN_FMW_Code_ErrorExitValue("Zen_GetCachedUnitsWaypoints", "Invalid cache identifier given.", [])
};

_array = Zen_Cache_Array select (_indexes select 0);

call Zen_StackRemove;
+([_array select 3, _array select 4])
