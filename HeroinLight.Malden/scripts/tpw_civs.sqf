...
// READ IN VARIABLES
tpw_civ_version = "1.54"; // Version string
...
tpw_civ_mapstrings = _this select 11;
...
[tpw_civ_mapstrings] call tpw_core_fnc_civs;
...
tpw_civ_fnc_nearciv =
	...
	if (_shareflag == 0 && {count tpw_civ_houses > 2}) then 
		{
...
0 = [] spawn 
	...
	while {true} do
		...
		if (tpw_civ_oldpos distance position player > (tpw_civ_radius / 2)) then
			...
			tpw_civ_houses = [tpw_civ_radius] call tpw_core_fnc_houses;
		...
		if (count tpw_civ_civarray > 0) then
			...
			for "_i" from 0 to (count tpw_civ_civarray - 1) do
				...
				if !(alive _civ) then 
					...
					if (((stance _civ == "CROUCH") ||(_civ distance vehicle player > tpw_civ_radius)) && {(lineintersects [eyepos player,getposasl _civ,player,_civ]) || (terrainintersectasl [eyepos player,getposasl _civ])}) then
						{
