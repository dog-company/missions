...
Date: 20170917
Version: 1.20
Requires: CBA A3, tpw_core.sqf, tpw_ambience.pbo, tpw_sounds.pbo
...
2 - Call it with 0 = [1,1,1,1,1,0,0,0,12] execvm "tpw_soap.sqf";  
...
if (count _this < 9) exitwith {hint "TPW soap incorrect/no config, exiting."};

// READ IN CONFIGURATION VALUES
tpw_soap_version = "1.20"; // Version string
...
tpw_soap_mideastsoundlength = [52,64,23,32,28,40,81,9,15,81,64,63,59,97,101,90,66,21,52,30,30,140]; // Length (sec) of each ambience clip in the order of the config
...
private _maps = ["altis","stratis","malden","lythium","tanoa","prei_khmaoch_luong","dya"];
private _adj = [1,1.5,1.5,1.5,1.2,0.2,0.5];
...
tpw_soap_fnc_housescan =
	...
	if (tpw_soap_region == 2) then
		...
		tpw_soap_sounds = 22;
		tpw_soap_songs = 11;
		tpw_soap_path = "TPW_AMBIENCE\sounds\mideast\";
	...
	if (tpw_soap_region == 4) then
		...
		tpw_soap_sounds = 29;
		tpw_soap_songs = 12;
		tpw_soap_path = "TPW_AMBIENCE\sounds\france\";
	...
	if (tpw_soap_region == 5) then
		...
		tpw_soap_sounds = 29;
		tpw_soap_songs = 8;
		tpw_soap_path = "TPW_AMBIENCE\sounds\russia\";
	...
	if (tpw_soap_region == 6) then
		...
		tpw_soap_path = format ["%1%2\","TPW_AMBIENCE\sounds\",["greece","mideast","fiji","france","russia"] select _rnd];
...
tpw_soap_fnc_constructionsounds =
	...
	[_house] spawn	
		...
		_clip = format ["%1%2.ogg",'TPW_AMBIENCE\sounds\construction\b',_sel + 1];	
...
tpw_soap_fnc_azanplay =
	...
	[_mosque] spawn	
		...
		_song = format ["TPW_AZAN\sounds\%1.ogg",(_sel + 1)];
		...
		playsound3d ["TPW_AMBIENCE\sounds\mideast\18.ogg",nearestbuilding player];
		waituntil
			{
			sleep 10;
	...
	while {true} do
		...
		if (tpw_soap_active && {daytime > tpw_soap_sunrise} && {daytime < tpw_soap_sunset}) then
			...
			if (count nearestTerrainObjects [_randpos, ["tree"], 50, false]> 50) then
				{
				_sound = format ["TPW_SOUNDS\sounds\chainsaw%1.ogg",(ceil random 4)];
				playsound3d [_sound,_randpos,false,_randpos,10+random 5,1,500];
				};
			};
		};
	};	
	...
	while {true} do
		...
		if (tpw_soap_active) then
			...
			if (count _buildings > 1) then
				...
				_vol = tpw_soap_mapfactor * tpw_soap_trafficmult * _roads * _roads * tpw_soap_soundvolmult;
				if (_vol > tpw_soap_trafficmult) then {_vol = tpw_soap_trafficmult};
				_sound = format ["TPW_AMBIENCE\sounds\traffic\%1.ogg",(ceil random 3)];
				//hint format ["Roads:%1, Vol:%2, Dist: %3 Sound:%4", _roads,_vol,_building distance player,_sound];
	...
	if ((tpw_soap_active) && {diag_ticktime > tpw_soap_nextcry} && {random 10 < 3}) then 
		...
		if ((_nearhouses select 0 distance player  < tpw_soap_radius) && {!([] call tpw_soap_fnc_indoors)}) then
			...
			if (count ([100] call tpw_core_fnc_houses) > 20) then
				{
				sleep random 10;
				_sound = format ["TPW_SOUNDS\sounds\siren%1.ogg",ceil (random 4)]; // sirens
